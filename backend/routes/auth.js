...
router.post("/send-otp", otpLimiter, async (req, res) => {
  try {
    ...
    // If user doesn't exist yet, create them (role defaults to 'customer')
    if (!user) {
      user = await User.create({ name: name || "Guest", email, authMethod: "otp" });
    }

    // Block non-admins before even sending OTP
    if (user.role !== "admin") {
      return res.status(403).json({ message: "Access denied. This dashboard is for admins only." });
    }
...
router.post("/verify-otp", authLimiter, async (req, res) => {
  try {
    ...
    // Double-check role at verify step too
    if (user.role !== "admin") {
